...
num_ics = 9;
exp = duff_det_forced_cos( ...
	num_ics, ... # of initial conditions
	round(normrnd(20,2,num_ics,1)), ... # final time of all experiments
	10); % point multiplier sets the sampling time number of points = mult*tfin + 1
...
tas_pq = pqEDMDm(p=5, ... [2 3 4 5], ... 
	...
	dyn_dcp = @(obs,data)sidDecomposition(10,0,obs,data)); % ''  to use the ordinary least squares
